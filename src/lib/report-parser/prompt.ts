 ...
 */
export function buildExtractionPrompt(threadText: string, todayIso: string): string {
  return `You are extracting structured data from a youth/adult pickup soccer league report email thread called "Kaiser". Today's date is ${todayIso} — use this only for inferring a missing year (rule 11), nothing else. Follow these rules exactly:
...
6. Notable mentions: capture any standout narrative language about a specific player's performance (e.g. "X was dominant on both ends" or "Y went through 4-5 players"), even for a player who didn't score — this is exactly the kind of qualitative signal that a pure goal-count would miss. Quote the relevant snippet verbatim. When one sentence credits TWO OR MORE players jointly (e.g. "Russell and Emre totally controlled the middle of the field"), emit a SEPARATE notableMentions entry per player named, each with the same quote — playerRaw must always be exactly one person's name, never "X and Y" combined into one string.
